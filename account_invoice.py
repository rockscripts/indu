...
class account_invoice(models.Model):
    ...
    def invoice_validate(self):
        ...
        if(eDocumentType=="FAC" or eDocumentType=="INV" or eDocumentType=="EFACT" or eDocumentType=="FACT"):
            for invoice in self:      
                ...
                for item in items:
                    ...
                    if(float(item.discount/100) > 0.0):
                        subTotalVenta = float(subTotalVenta) - float(float(subTotalVenta) * float(item.discount/100))

                    # global discount >> point of sale config
                    if(bool(item.product_id.sunat_is_product_discount) == True):
                        sunat_discount_type = str(item.product_id.sunat_discount_type).split("-")
                        sunat_discount_type_code = str(sunat_discount_type[0]).strip()
                        sunat_discount_type_text = str(sunat_discount_type[1]).strip()

                        item_global_discount = {
                                                    "codigo": sunat_discount_type_code,
                                                    "razon": sunat_discount_type_text,
                    ...
                    sunat_item = {
                                    ...
                                    'clasificacionProductoServicioCodigo':self.get_sunat_product_code_classification( item.product_id.sunat_product_code, item.product_id.name),
                    ...
                    if(float(subTotalVenta)>0.00001):
            ...
            sunat_data = {
                                ...
                                "descuentos": {"globales": global_discounts},
                                "sunat_sol": {
            ...
            #f = open("/projects/odoope12/custom/addons/sunat_fact/r.json","a")
            #f.write(str(sunat_data))
            #f.close()
            ...
            try:
                ...
                self.sunat_request_type = sunat_request_type
                self.sunat_total_letters = self.literal_price(totalVentaPedido)
                self.digest_value = self.get_hash_code()
            except:
                pass
...
################################################################################################
            ...
            #f = open("/projects/odoope12/custom/addons/sunat_fact/service_sol/log.json","a")
            #f.write(str(sunat_data))
            #f.close()
...
################################################################################################
            ...
            try:
                ...
                qr_code_text = str(self.number) + str("|") + str(self.date_invoice)
                qr.add_data(qr_code_text)
                qr.make(fit=True)
                img = qr.make_image()
                temp = BytesIO()
    ...
    def get_sunat_document_type(self, journal_code):
        if journal_code == "FAC" or journal_code == "INV"  or journal_code == "EFACT":
            return str("01")
        if journal_code == "BOL":
            return str("03")
        if journal_code == "TCV":
    ...
    def get_sunat_product_code_classification(self, classification, product_name):       
        try:            
            sunat_product_code_parts = str(classification).split(" -- ")
            return sunat_product_code_parts[0]
        except:
            
            raise Warning("El producto " + str(product_name) + str(" no tiene asignado la clasificacion de producto o servicio."))
    ...
    def get_hash_code(self):
        ...
        try:
            ...
            if(len(serieParts)):
                ...
                tree = etree.parse(xmlPath+'/XMLdocuments/3_signed/'+str(fileName)+".XML")
                MainNode = tree.getroot()
                UBLExtensions = MainNode.find('{urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2}UBLExtensions')
                UBLExtension = UBLExtensions.find('{urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2}UBLExtension')
                ExtensionContent = UBLExtension.find('{urn:oasis:names:specification:ubl:schema:xsd:CommonExtensionComponents-2}ExtensionContent')
                Signature = ExtensionContent.find('{http://www.w3.org/2000/09/xmldsig#}Signature')
                SignedInfo = Signature.find('{http://www.w3.org/2000/09/xmldsig#}SignedInfo')
                Reference = SignedInfo.find('{http://www.w3.org/2000/09/xmldsig#}Reference')
                DigestValue = Reference.find('{http://www.w3.org/2000/09/xmldsig#}DigestValue')
